...
export const FAQ = () => (
  <div>
    ...
    <p>open.mp (Open Multiplayer, OMP) to zastępczy tryb multiplayer dla San Andreas, zainicjowany w odpowiedzi na niefortunny wzrost problemów z aktualizacjami i zarządzaniem klienta SA-MP. Wstępna wersja będzie zamiennikiem tylko dla serwerów. Istniejące klienty SA-MP'a będą mogły łączyć się do naszych nowych serwerów. W przyszłości, nowy klient open.mp stanie się dostępny, który pozwoli na udostępnianie bardziej interesujących aktualizacji.</p>
    <hr />
    <h2>Czy to jest kolejny 'fork'?</h2>
    <p>Nie. Jest to kompletnie pisane na nowo oprogramowanie, wykorzystując przy tym dziesięciolecia wiedzy i doświadczenia. Podejmowano wcześniej próby rozwidlenia (fork) SA-MP'a, ale uważamy, że miały one dwa główne problemy:</p>
    <ol>
    ...
    <p>
      Pomimo licznych prób w przyspieszeniu rozwoju SA-MP'a drogą oficjalną, w formie sugestii czy ofert pomocy od zespołu Beta; wraz z całą społecznością wołając o "coś" nowego; nie zaobserwowano żadnego postępu od paru lat. Powszechnie uważano, że wynikało to z powodu braku zainsteresowania ze strony właściciela trybu multiplayer, co nie jest problemem samo w sobie, ale nie było żadnej linii sukcesji. Zamiast przekazać rozwój projektu wszystkim tym, którzy są tym zainteresowani i chcą kontynuować pracę nad modem, założyciel chciał po prostu sprowadzić wszystko na siebie, pozornie łącząc rzeczy tak długo, jak to możliwe, przy minimalnym wysiłku. Niektórzy twierdzą, że jest to powiązane z pasywnym dochodem, ale niestety nie ma na to dowodów. Pomimo ogromnego zainteresowania i silnej, rodzinnej społeczności, wierzył, że mod przetrwa zaledwie rok lub dwa, a społeczność, która tak ciężko pracowała, aby uczynić SA-MP'a tym czym jest dzisiaj (jego zdaniem) nie zasługują na aktualizację i
      nowe zmiany.
    </p>
    <br />
    ...
    <p>burgershot.gg jest to forum o grach, nic więcej. Wiele osób jest zaangażowanych w oba tematy, niektóre prace OMP i aktualizacje są tam również publikowane, ale są to dwa niezależne projekty. Nie są to fora o OMP ani OMP nie jest własnością burgershot. Po uruchomieniu pełnej strony OMP, obie strony mogą zostać wyodrębnione (tak jak SA-MP był kiedyś hostowany przez GTAForums, zanim jego własna strona została uruchomiona).</p>
    <hr />
    <h2>Czym jest OpenMP?</h2>
    <p>Projekt Open Multi-Processing to "OpenMP", my jesteśmy "open.mp". Kompletnie czymś innym.</p>
  </div>
);
