...
* **Fix the warnings:**  Some functions have changed names for consistency with each other; some
...
* it isn't an invalid index, so when returned and not correctly checked your code won't crash.  It
...
* It IS an invalid index.  Not being one was listed as an advantage for `0` because your code will
...
* People are used to `0` as a valid value most of the time.  Programmers start counting at `0`, so
it should be valid and something outside the realm of positive integers should be invalid.

* In unsigned maths it is the largest possible integer - `0xFFFFFFFF`, `4294967295`.  This means
